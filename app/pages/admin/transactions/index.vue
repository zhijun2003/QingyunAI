<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold">交易管理</h2>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <NCard>
        <div class="text-center">
          <div class="text-sm text-gray-500 mb-2">总交易数</div>
          <div class="text-2xl font-bold">{{ stats.totalCount }}</div>
        </div>
      </NCard>
      <NCard>
        <div class="text-center">
          <div class="text-sm text-gray-500 mb-2">总充值</div>
      ...
      <NCard>
        <div class="text-center">
          <div class="text-sm text-gray-500 mb-2">总消费</div>
      ...
      <NCard>
        <div class="text-center">
          <div class="text-sm text-gray-500 mb-2">净收入</div>
          <div class="text-2xl font-bold text-blue-600">
            ¥{{
              formatAmount(
                (stats.byType.RECHARGE?.amount || 0) +
                  (stats.byType.CONSUMPTION?.amount || 0)
              )
            }}
          </div>
        </div>
    ...
    <NCard class="mb-6">
      <NSpace>
        ...
        />
        <NSelect
          v-model:value="filter.paymentMethod"
...
const columns: DataTableColumns<any> = [
  {
    ...
    width: 150,
    render(row) {
      return row.user?.username || row.user?.email || row.user?.phone || '-'
    },
  },
  {
    title: '交易类型',
...
async function loadTransactions() {
  ...
  try {
    ...
    const response = await get('/api/admin/transactions', query)
    if (response.success) {
